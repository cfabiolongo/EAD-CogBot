...
adapters_name1 = f"../models/finetuned/llama-fol_50ep"
adapters_name2 = f"../models/finetuned/llama-dolly_qa_100ep"

adapters_name_unified = f"../models/finetuned/llama-cogbot"
...
model.load_adapter(adapters_name2, adapter_name="dolly")

model.add_weighted_adapter(["fol", "dolly"], [w1,w2], combination_type=comb_type, adapter_name="dolly_fol")

model.delete_adapter("fol")
model.delete_adapter("dolly")
model.save_pretrained("../models/finetuned")

model = PeftModel.from_pretrained(model, "../models/finetuned/dolly_fol")
...
for i in range(len(dataset)):
    ...
    print(f"\nPrompt {i} :\n{d['response']}\n")
    print(f"Generated instruction ({len(gen)}):\n{gen}")
